...
from py_myt.exceptions import MYTSDKError, MYTSDKDownloadError, MYTSDKFileError, MYTSDKProcessError
...
class TestMYTSDKManager(unittest.TestCase):
    ...
    def test_is_sdk_running_true(self, mock_process_iter):
        """测试SDK正在运行的情况"""
        mock_process = Mock()
        mock_process.info = {'name': 'myt_sdk.exe'}
    ...
    def test_download_sdk_success(self, mock_get, mock_zipfile):
        ...
        # 模拟ZIP文件
        mock_zip = Mock()
        mock_zip.namelist.return_value = ['myt_sdk.exe', 'config.json']
    ...
    def test_init_already_running(self, mock_installed, mock_running, mock_download, mock_start):
        """测试初始化时SDK已在运行"""
    ...
    @patch.object(MYTSDKManager, 'is_sdk_running')
    @patch.object(MYTSDKManager, 'is_sdk_installed')
    def test_init_download_and_start(self, mock_installed, mock_running, mock_download, mock_start):
        """测试初始化时需要下载和启动"""
