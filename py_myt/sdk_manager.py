    ...
    def is_sdk_running(self) -> bool:
        ...
        try:
            for proc in psutil.process_iter(['pid', 'name', 'exe']):
                try:
                    if proc.info['name'] and self.SDK_EXECUTABLE.lower() in proc.info['name'].lower():
                        return True
                    if proc.info['exe'] and str(self.sdk_executable_path).lower() in proc.info['exe'].lower():
                        return True
    ...
    def _extract_sdk(self, zip_path: Path) -> None:
        ...
        try:
            ...
            if not exe_found or not self.sdk_executable_path.exists():
                ...
                logger.error(f"解压后的文件列表: {all_files[:20]}...")  # 只显示前20个文件
